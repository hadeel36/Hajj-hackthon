...
class TripsController extends Controller
{
    ...
    public function actionGetTrips()
    {
        ...
        if (isset($_POST['qr_code'])) {
            $qrDetails = explode('_', $_POST['qr_code']);
            $account = AccountHelper::findAccount($qrDetails[0], $qrDetails[1]);
            if (is_object($account)) {
                $accountType = AccountHelper::getAccountType($_POST['qr_code']);
                $trips = TripsHelper::getAccountTrips($account, $qrDetails[1]);
                return json_encode([
                    'success' => true,
                    'data' => ['trips' => $trips, 'account_type' => $accountType]
                ]);
            }
        }
        return json_encode([
